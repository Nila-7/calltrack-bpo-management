...
export default function DocumentViewer() {
  ...
  const simulateBreach = async () => {
    ...
    toast({
      variant: "destructive",
      title: "⚠ Deception Protocol Triggered",
      description: "Unauthorized access profile detected. Decoy document served.",
    })
  }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="h-16 border-b bg-white flex items-center justify-between px-8 shrink-0">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Badge variant="outline" className="text-[10px] uppercase font-bold text-slate-400">Identity-Protected</Badge>
          </div>
        </div>

        <div className="flex items-center space-x-4">
          {suspiciousMode && (
            <div className="bg-red-50 text-red-600 border border-red-200 px-3 py-1 rounded-full text-xs font-bold animate-pulse flex items-center uppercase tracking-wider">
              <AlertTriangle className="w-4 h-4 mr-2" /> Deception Shield Active
            </div>
          )}
          <Button 
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-8 overflow-auto flex justify-center bg-slate-100">
          <Card className="w-full max-w-3xl h-fit min-h-full border-none shadow-2xl bg-white rounded-none md:rounded-lg">
            <CardContent className="p-12 font-mono text-sm leading-relaxed whitespace-pre-wrap select-text selection:bg-primary/20">
              <div className="mb-8 flex justify-between border-b pb-4">
                <div className="text-[10px] text-slate-400 uppercase tracking-[0.2em] font-bold">Document Stream</div>
                {viewMode === 'decoy' ? (
                  <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-200 border-amber-200 font-bold text-[10px] uppercase">
                    <ShieldCheck className="w-3 h-3 mr-1" /> Deception Mode Activated
                  </Badge>
                ) : (
                  <Badge variant="outline" className="text-slate-400 font-bold text-[10px] uppercase">Original Data View</Badge>
                )}
              </div>
              <div className="bg-slate-50 p-8 border rounded-sm">
        ...
        <aside className="w-[400px] bg-white border-l p-6 overflow-auto space-y-6">
          <div className="space-y-4">
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500">Unmodified structural data</div>
                  </div>
                </div>
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500">Serves deceptive PII entities</div>
                  </div>
                </div>
            ...
            >
              ...
              {showClassification ? "Hide Classification" : "Entity Classification Map"}
            </Button>
          </div>
          ...
          {showClassification && (
            <div className="space-y-4 animate-in slide-in-from-right duration-300">
              <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 flex items-center">
                <Info className="w-4 h-4 mr-2 text-primary" /> Deception Analysis
              </h3>
              <div className="border rounded-lg overflow-hidden bg-white shadow-sm">
                <Table>
                  <TableHeader className="bg-slate-50">
                    <TableRow>
                      <TableHead className="text-[9px] uppercase font-black tracking-tighter">Entity</TableHead>
                      <TableHead className="text-[9px] uppercase font-black tracking-tighter">Original</TableHead>
                      <TableHead className="text-[9px] uppercase font-black tracking-tighter">Decoy</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {detections?.map((entity, i) => (
                      <TableRow key={i} className="text-[10px]">
                        <TableCell className="font-bold text-primary py-2">{entity.key}</TableCell>
                        <TableCell className="text-slate-400 py-2 truncate max-w-[80px]">{entity.originalValue}</TableCell>
                        <TableCell className="text-emerald-600 font-black py-2 truncate max-w-[80px]">{entity.decoyValue}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          {!showClassification && (
            <div className="bg-slate-50 p-6 rounded-xl border border-dashed border-slate-200 text-center">
              ...
              <p className="text-[10px] text-slate-400 leading-relaxed font-medium">
                Identity analysis engine has classified {detections?.length || 0} sensitive entities within the document structure.
              </p>
            </div>
          )}
        </aside>
